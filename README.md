...
_UID仅仅通过杂凑运算得到，对于数据分析而言并无作用，为安全起见，请不要泄漏给任何人。_
...
**备注**：RID 指 Record Identity Number，是这个项目生成的，仅仅用于掩盖原始的抽卡记录ID以及个人范围内的数据去重。它同样是通过杂凑运算得到的，为了安全起见，请在分享抽卡记录前将此字段去除。
