...
抽卡记录指的是原神的祈愿卡池的抽取历史记录，而这是一个用于管理抽卡记录的Python库，它用类的方式封装实现了以下功能，而你只需要实例化区区几个类，就可以很轻易地实现这些功能。
...
- [ ] 将日志读取方法内的日志路径独立为参数并公开，方便其他平台探索。



## 数据兼容性
项目还没有正式发布，所以导出的数据格式没有作向后兼容。以下是一些解决之策：
...
首先是可以正常拉取抽卡记录，然后用拉取到的部分按日期时间覆盖掉原先的记录。
因为RID的编码空间比id大，所以可以将剩余未被覆盖的记录的键名"RID"替换为"id"，就能正常被载入。
...
如果你的代码版本还处于v0.1，那将多出的字段删除，再将键名"id"替换为"RID"即可。

如果你的代码版本已经是v0.2，那么直接删除多出的字段即可。
